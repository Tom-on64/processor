...
void command(state_t* state, char* command, char* out) {
    ...
    if (tokenCount == 0) {
        ...
        strcpy(out, "\x1b[H\x1b[J"); // Clear screen
    } else if (strcmp(tokens[0], "dump") == 0) {
        if (tokenCount < 2) {
            strcpy(out, "Expected address!\n");
            return;
        }
        ...
        if (tokenCount >= 3) len = (uint16_t)strtoul(tokens[2], NULL, 0);
        ...
        for (int i = 0; i < len; i++) {
            ...
            printf("%02x ", state->memory.raw[addr + i]);
        }
        printf("\n");
        ...
        strcpy(out, "EMU " VERSION " monitor help:\n"
                ...
                " dump <addr> [num] Prints num bytes at address (256 by default)\n"
                " exit              Stops the emulator and exits\n"
                " goto <addr>       Sets PC to address\n"
                " halt              Toggles the halt bit\n"
                " help              Prints this help message\n"
                " load <file> [loc] Loads a file into loc (0x0000 by default)\n"
        ...
        if (tokenCount < 2) {
            strcpy(out, "Expected filename!\n");
            return;
        }

        uint16_t addr = 0;
        if (tokenCount >= 3) addr = strtoul(tokens[2], NULL, 0);
        
        if (load(state, tokens[1], addr) != 0) {
            sprintf(out, "%s not found!\n", tokens[1]);
            return;
        }

        sprintf(out, "%s loaded at 0x%04x\n", tokens[1], addr);
        ...
        if (tokenCount < 3) {
            strcpy(out, "Expected address/value!\n");
            return;
        }
